...
int writeFlowerOrdersToFile(Flower* flowerorderdb, int count, int Id) {
    ...
    for (int i = 0; i < count; i++) {
        if(flowerorderdb[i].getFlowerID() == Id){
        file << flowerorderdb[i].getFlowerID() << " " << flowerorderdb[i].getName() << " " << flowerorderdb[i].getPrice() << " " <<static_cast<int> (flowerorderdb[i].getType()) << " " << flowerorderdb[i].getStock()  << std::endl;
...
bool initializestandardUsers(std::vector<User>& users) {
    // Open the user file for reading
    std::ifstream userFile("users.txt");
    if (!userFile) {
        return false; // Unable to open the file
...
bool initializeadmins(std::vector<admin>& admins) {
    // Open the user file for reading
    std::ifstream userFile("users.txt");
    if (!userFile) {
        return false; // Unable to open the file
    ...
    while (userFile >> username >> password >> isAdmin) {
        // Use the constructor to create a UserBase object and add it to the vector
        admins.emplace_back(username, password, isAdmin);
    }
...
void placeOrder(struct Flower flowerDatabase[], int flowerCount) {
    ...
    if (foundIndex >= 0) {
        if (flowerDatabase[foundIndex].getStock() > 0) {
            ...
            writeFlowerOrdersToFile(flowerorderdb, flowercount,flowerID);
...
void showorderbyusername(const std::vector<UserBase>& users, const std::string& usernam) {
...
bool WriteAdminToTxtFile(const std::vector<admin>& admins) {
    std::ofstream file("admins.txt");
    if (file.is_open()) {
        for (const admin& adminc : admins) {
            file << adminc.getUsername() << " " << adminc.getPassword() << " " << adminc.isAdminUser() << std::endl;
        }
        file.close();
        std::cout << "Admin information written to file: " << "admins.txt" << std::endl;
        return true;  // Başarılı oldu, true döndürüyoruz.
    } else {
        std::cerr << "Unable to open file: " << "admins.txt" << std::endl;
...
bool WriteUsereToTxtFile(const std::vector<User>& users) {
    std::ofstream file("user.txt");
    if (file.is_open()) {
        ...
        file.close();
        std::cout << "User information written to file: " << "users.txt" << std::endl;
        return true;  // Başarılı oldu, true döndürüyoruz.
    } else {
...
void deleteUser(std::vector<UserBase>& users, int* userCount, const char* deleteUsername,std::vector<admin>& admins, int adminCount, std::vector<User>& userss, int usercount){
    ...
    void deleteUser(std::vector<UserBase>& users, int* userCount, const char* deleteUsername, std::vector<admin>& admins, int adminCount, std::vector<User>& userss, int usercount) {
    int found = 0;
    int isadminUser = -1; // Initialize with an invalid value
    ...
    for (auto it = users.begin(); it != users.end(); ++it) {
        if (it->getUsername() == deleteUsername) {
            ...
            isadminUser = it->isAdminUser();
             if (isadminUser == 0) {
        deletestandartuser(userss, userCount, deleteUsername);
    } else if (isadminUser == 1) {
        deleteadmin(admins, &adminCount, deleteUsername);
    }
            it = users.erase(it); // Erase the element and update iterator
            (*userCount)--;
            break; // Exit the loop
        }
    }
